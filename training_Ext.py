...
from attentivefp.utils import chem, log, data, splitter, tuning, TabularPP
from attentivefp.featurizer import graph
from attentivefp.models.dgl import AttentiveFPDense_Ext, collate_molgraphs_Ext, EnsembleAttFP_Ext
from attentivefp.models import baseline
from attentivefp.models.training import perform_cv_Ext, training_dataloader_Ext
...
def parse_arguments():
    ...
    parser.add_argument('--split', help='CrossValidation Split Type (Default: KFold)', action='store', required=False,
                        default="KFold",
                        choices=['KFold', 'Random', 'Butina', 'Scaffold', 'GroupKFold', 'Predefined', 'Cumulative'])
    parser.add_argument('--split_column',
                        help='Column with splitting data. Only relevant for split types "GroupKFold" and "Predefined"',
                        action='store', required=False, dest='split_column')
    ...
    parser.add_argument('--hyper_evals', help='Hyperopt max_evals. 0 disabled hyperoptimization and uses defaults',
                        type=int, default=0)
    ...
    parser.add_argument('--pretrained_model', help='pth model file location of trained AttentiveFPDense model',
                        required=False)
    parser.add_argument('--seed', help='Seed for data randomization, cross validation, boot strapping, and torch',
...
def main(args):
    ...
    if args.tab_postproc:
        tab_pp_file = args.tab_postproc if os.path.isabs(args.tab_postproc) else os.path.join(os.getcwd(), args.tab_postproc)
    ...
    df_prep, task_labels, mask_missing, tab_prep = data.preprocess_Ext(df, smiles_cols, task_cols, id_col, grp_col, args.standardize,
                                                         random_state=args.seed, df_tab=df_tab, tab_pp=tab_pp)
    del df, df_tab, tab_pp
    ...
    graphs = [dglf.featurize_mols(mols) for mols in lst_mols]
    ...
    hyperparameters = {'node_feat_size':  atomFeatureSize,  # 74 for canonical
                       ...
                       'tab_feat_size':   len(tab_prep[0]) if args.tab_file else 0,
                       'num_layers':      3,
                       'num_timesteps':   2,
                       'graph_feat_size': 200,
                       'dropout':         0.2,
    ...
    if args.pretrained_model is not None:
        ...
        model.predict = nn.Sequential(nn.Dropout(p=0.2, inplace=False),
                                      nn.Linear(in_features=model.predict[1].in_features,
                                                out_features=hyperparameters['n_tasks'], bias=True))
    else:
    ...
    if args.split != "None" and not args.skip_cv:
        train_dl = perform_cv_Ext(model, optimizer, task_labels, mask_missing, tab_prep, cvFolds,
                              nn.SmoothL1Loss(reduction='none'), *graphs, max_epochs=args.max_epochs,
        ...
        with open(os.path.join(save_dir, 'cv_results.txt'), 'w') as _output_file_writer:
            for k, d in train_dl.items():
                ...
                cv_df = pd.DataFrame(np.concatenate([d['test_y'], d['test_preds'], d['test_std']], axis=1),
                                     columns=[f'{c}' for c in task_cols] + [f'{c}:pred' for c in task_cols] + [
                                         f'{c}:std' for c in task_cols])
                ...
                cv_df.to_csv(_output_file_writer, sep='\t', index=False, float_format='%.3f',
    ...
    if args.baseline:
        # Currently only considering the first molecule
        logger.warning('The baseline is only created using features for the first SMILES column')
        bsl_r = baseline.get_baseline(lst_mols[0], task_labels.numpy(), mask_missing.numpy(), cvFolds,
                                      metrics=[mean_squared_error, r2_score, median_absolute_error])
    ...
    if not args.skip_final:
        logger.info(f'Training final model on all {len(graphs[0])} datapoints')
        # Create data loader
        final_dataloader = DataLoader(
            list(zip(task_labels, mask_missing, tab_prep, *graphs)),
        ...
        summary = training_dataloader_Ext(model, optimizer, final_dataloader, loss_fn=nn.SmoothL1Loss(reduction='none'),
        ...
        ensemble = EnsembleAttFP_Ext(models=ensemble_models)
        torch.save(
            ensemble,
...
if __name__ == '__main__':
    ...
    assert all([l in df.columns for l in args.task_cols]),   'Required task column missing in input'
    assert all([l in df.columns for l in args.smiles_cols]), 'Required smiles column missing in input'
