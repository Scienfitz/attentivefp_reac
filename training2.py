...
def main(args):
    ...
    df_prep, task_labels, mask_missing = data.preprocess(df, smiles_cols, task_cols, id_col, grp_col, args.standardize,
    ...
    mols1 = df_prep['_mol0'].values
    mols2 = df_prep['_mol1'].values
    mols3 = df_prep['_mol2'].values
    ...
    if not args.skip_cv or args.hyper_evals or args.baseline:
        ...
        cvFolds = splitter.getSplit(mols1, task_labels, split=args.split, n_splits=args.split_n, groups=grp,
                                    random_state=args.seed)
    ...
    graphs1 = dglf.featurize_mols(mols1)
    graphs2 = dglf.featurize_mols(mols2)
    graphs3 = dglf.featurize_mols(mols3)
    ...
    if args.pretrained_model is not None:
        ...
        for param in model.attfp1.parameters():
            param.requires_grad = False
        for param in model.attfp2.parameters():
            param.requires_grad = False
        for param in model.attfp3.parameters():
            param.requires_grad = False
    ...
    if args.split != "None" and not args.skip_cv:
        train_dl = perform_cv2(model, optimizer, graphs1, graphs2, graphs3, task_labels, mask_missing, cvFolds,
    ...
    if args.baseline:
        ...
        bsl_r = baseline.get_baseline(mols1, task_labels.numpy(), mask_missing.numpy(), cvFolds,
                                      metrics=[mean_squared_error, r2_score, median_absolute_error])
    ...
    if not args.skip_final:
        logger.info(f'Training final model on all {len(graphs1)} datapoints')
        # Create data loader
        ...
        ensemble = EnsembleAttFP2(models=ensemble_models)
        torch.save(
            ensemble,
...
if __name__ == '__main__':
    ...
    print('\n\n',args.task_cols, args.smiles_cols, '\n\n')
    ...
    assert len(args.smiles_cols)==3, 'There need to be exactly 3 SMILES columns'
