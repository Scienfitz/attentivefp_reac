...
class AttentiveFPDense_Ext(nn.Module):
    # An Extension to add support for several separated graphs and tabular features
    def __init__(self,
                 node_feat_size,
                 edge_feat_size,
                 n_graphs,
        ...
        super(AttentiveFPDense_Ext, self).__init__()
        ...
        if n_dense > 0:
            ...
            for d in range(n_dense):
                self.dense.append(
                    nn.Sequential(
                        nn.Dropout(dropout),
                        nn.Linear(self.n_units if d > 0 else interface_layer_size, self.n_units),
                        nn.ReLU()
                    )
                )
            self.dense = nn.ModuleList(self.dense)
        ...
        self.predict = nn.Sequential(nn.Dropout(dropout),
                                     nn.Linear(self.n_units if self.n_dense > 0 else interface_layer_size,
                                               self.n_tasks)
                                     )
...
class EnsembleAttFP_Ext(nn.Module):
    def __init__(self, models):
        super(EnsembleAttFP_Ext, self).__init__()
        self.models = nn.ModuleList(models)

    def forward(self, *args):
        output = torch.stack([model(*args) for model in self.models], dim=0)
        return output

def collate_molgraphs_Ext(data):
    # Adjusted version, adding the capability of multiple graphs and tab features
    """Batching a list of datapoints for dataloader.
    Parameters
    ----------
    data : list of 3-tuples or 4-tuples.
